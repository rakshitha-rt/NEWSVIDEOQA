...
#         self.eval()
#         out_dict = self.forward(val_batch)
#         # self.log('Predictions: ',out_dict['predictions'])
#         xml = []
#         for i in range(len(out_dict['predictions'])):
#             cap = self.tokenizer.decode(out_dict['predictions'][i], skip_special_tokens=True)
#             xml.append({'question_id': out_dict['question_id'][i].item(), 'caption': cap})
#         return xml
    
#     def validation_epoch_end(self, validation_step_outputs):
#         xml = []
#         for i in range(len(validation_step_outputs)):
#             for j in range(len(validation_step_outputs[i])):
#                 xml.append(validation_step_outputs[i][j])
#         xx = json.dumps(xml)
#         with open('all_predicted_vqa.json', 'w') as w:
#             w.write(xx)
#         print(xml)
...
#             start_predictions, decoding_step, **search_param
#         )
#         # print('keys - - - - - - - - - - - - -', self.tokenizer.decode(batch['caption'][0]))
#         # print('np - - - - - - - - - - - - -', self.tokenizer.decode(batch['caption_tokens'][0]))
...
#         # loss = self.loss(predicted_caption, batch['caption'])
#         # print(loss)
#         # input()
#         output_dict = {
#             'predictions': predicted_caption,
...
#                 if self.prev_encoded_layers is None:
#                     self.prev_encoded_layers = logits[1]
#                     # print('hello', self.prev_encoded_layers.shape)
#                 else:
#                     self.prev_encoded_layers = [torch.cat((p, c), dim=1) for p, c in
#                                                 zip(self.prev_encoded_layers, logits[1])]
#                 #self.prev_encoded_layers = None
